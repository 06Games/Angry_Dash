...
public class LangueSelector : MonoBehaviour
{
    ...
    void GetLangDispo(bool forceUpdate = false)
    {
        ...
        for (int i = 0; i < languages.Length; i++)
        {
            ...
            if (File.Exists(Sprite_API.Sprite_API.spritesPath("native/GUI/languagesFlags/" + Path.GetFileNameWithoutExtension(languages[i]) + ".png")))
            {
                Texture2D tex = new Texture2D(1, 1);
                tex.LoadImage(File.ReadAllBytes(Sprite_API.Sprite_API.spritesPath("native/GUI/languagesFlags/" + Path.GetFileNameWithoutExtension(languages[i]) + ".png")));
